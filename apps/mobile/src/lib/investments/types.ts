...
/** Direct port of apps/web/src/lib/investments/types.ts. */
...
export const investmentInputSchema = z.object({
  ...
  investedAmount: z.coerce.number().min(0, "Invested amount can't be negative"),
  currentValue: z.coerce.number().min(0, "Value can't be negative"),
  monthlyContribution: z.coerce.number().min(0).optional().nullable(),
  expectedReturn: z.coerce.number().min(0, "Rate can't be negative").max(100, "That rate looks too high"),
