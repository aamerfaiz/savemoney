/**
 * Direct port of apps/web/src/lib/budgets/compute.ts — pure, unchanged
 * aggregation logic.
 */
...
export function computeBudgetsData(
  ...
  decryptedInvestmentMonthlyContributions: number[],
  currency: CurrencyCode,
  now = new Date(),
): BudgetsData {
  ...
  const investments = decryptedInvestmentMonthlyContributions.reduce((s, v) => s + v, 0);
