/**
 * Direct port of apps/web/src/lib/analytics/compute.ts — pure, unchanged.
 * No new Route Handler needed: every input here comes from
 * apiClient.finance.raw(), the same shared boundary Budget already reads.
 */
...
function deriveHealth(
  ...
  const variance = incomes.reduce((s, v) => s + (v - meanIncome) ** 2, 0) / (incomes.length || 1);
